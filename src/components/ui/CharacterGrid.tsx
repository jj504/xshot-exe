...
export default function CharacterGrid({ active = false, onLocked }: CharacterGridProps) {
  ...
  useEffect(() => {
    ...
    if (mq.matches) {
      ...
      cells.forEach(row => row.forEach(cell => { cell.resolved = true; cell.char = cell.target; cell.opacity = 1; }));
      ...
      for (let r = 0; r < ROWS; r++) {
        for (let c = 0; c < COLS; c++) {
          ...
          if (phaseRef.current === "noise") {
            ...
            const rt = 1500 + (dist / maxD) * 2500;
            if (elapsed > rt) { cell.resolved = true; cell.char = cell.target; cell.opacity = 1; }
            else {
              if (elapsed - cell.cycleTimer > 60) { cell.char = SCRAMBLE_SET[Math.floor(Math.random() * SCRAMBLE_SET.length)]; cell.cycleTimer = elapsed; }
              cell.opacity = 0.4 + 0.6 * Math.max(0, (elapsed - rt + 500) / 500);
            }
          }
          ...
          if (ch !== " ") {
            ...
            else if (line.includes("v2.1.3")) color = `rgba(176,196,196,${cell.opacity})`;
